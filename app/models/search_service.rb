...
class SearchService
  ...
  def save_activity
    @query_form_activity = Query.find_by(act_identifier: @session_identifier)
    @increment_query = Query.find_by(user_id: @user_id, query: @query)
    @similar_queries = Query.search(@query).user(@user_id).where(act_identifier: @session_identifier)
